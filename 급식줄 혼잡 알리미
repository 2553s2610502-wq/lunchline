...
# 예상 대기 시간 계산 (1명당 평균 12초 소요 가정)
# $ \text{대기 시간(분)} = \frac{\text{대기 인원} \times 12}{\text{60}} $
...
st.caption("식당에 계신 분들의 소중한 제보로 실시간 정보가 업데이트됩니다.")
...
with st.form(key="report_form", clear_on_submit=True):
    ...
    submit_button = st.form_submit_with_rows = st.form_submit_button(label="제보하기")
    ...
    if submit_button:
        try:
            ...
            st.success("제보가 성공적으로 반영되었습니다! 대시보드가 업데이트됩니다.")
            time.sleep(1)
            st.rerun()
            
        except Exception as e:
            st.error(content=f"데이터 처리 중 오류가 발생했습니다: {e}")
